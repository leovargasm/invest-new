...
$(document).ready(function(){
    ...
    (function( bootstro, $, undefined ) {
        ...
        var defaults = {
             // nextButtonText : 'Siguiente',&raquo;will be wrapped with button as below
            nextButton : '<button class="btn btn-success btn-xs bootstro-next-btn">Siguiente &raquo;</button>',
            /*prevButtonText : ' Anterior',*/
            prevButton : '<button class="btn btn-w btn-xs bootstro-prev-btn">&laquo; Anterior</button>',
            /*finishButtonText : '<i class="icon-ok"></i>Omitir',*/
            finishButton : '<button class="btn bootstro-finish-btn">Terminar</button>',
            ...
            /*onExit : function(params){
                alert('hola')
            },*/ //params = {idx : activeIndex}
            onStep : function(params){
                params = {idx : 0, direction : 'next'} 
                /*aler('hola');*/
                if(activeIndex == 2){
                    $('.solicitud:first-child .prestar .sxs-cta input.form-control').focusin();
                }
                if(activeIndex == 3 || activeIndex == 1){
                    $('.solicitud:first-child .prestar .sxs-cta input.form-control').focusout();
                }
            }, //params = {idx : activeIndex, direction : [next|prev]}
            //url : String // ajaxed url to get show data from
            
            margin : 200, //if the currently shown element's margin is less than this value
        ...
        get_popup = function(i)
        {
            ...
            p.template = '<div class="popover popu" style="' + style + '"><div class="arrow"></div><div class="popover-inner"><h3 class="popover-title"></h3><div class="popover-content"><p></p></div></div>' +
             '</div>';
